...
<div class="container">
  ...
  <form action="{{ route('pagos.update', $pago) }}" method="POST">
    ...
    <div class="mb-3">
      ...
      <select name="id_factura" id="id_factura" class="form-select" required>
        @foreach ($facturas as $factura)
          <option value="{{ $factura->id }}" {{ $pago->id_factura == $factura->id ? 'selected' : '' }}>
            {{ $factura->id }}
          </option>
        @endforeach
      </select>
    ...
    <div class="mb-3">
      ...
      <select name="id_tipo_pago" id="id_tipo_pago" class="form-select">
        ...
        @foreach ($tipopagos as $tipopago)
          <option value="{{ $tipopago->id }}" {{ $pago->id_tipo_pago == $tipopago->id ? 'selected' : '' }}>
            {{ $tipopago->nombre }}
          </option>
        @endforeach
      </select>
    ...
    <div class="mb-3">
      ...
      <input type="text" name="no_tarjeta" id="no_tarjeta" class="form-control" value="{{ $pago->no_tarjeta }}">
    </div>

    <div class="mb-3">
      <label for="no_cheque" class="form-label">No. Cheque</label>
      <input type="text" name="no_cheque" id="no_cheque" class="form-control" value="{{ $pago->no_cheque }}">
    </div>

    <div class="mb-3">
      <label for="cambio" class="form-label">Cambio</label>
      <input type="number" name="cambio" id="cambio" class="form-control" step="0.01" value="{{ $pago->cambio }}">
    </div>

    <div class="mb-3">
      <label for="fecha_expiracion" class="form-label">Fecha Expiración</label>
      <input type="date" name="fecha_expiracion" id="fecha_expiracion" class="form-control" value="{{ $pago->fecha_expiracion }}">
    </div>
